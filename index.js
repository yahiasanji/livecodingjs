...
function getAvailableDestinations() {
  return spaceData.destinations.filter((d) => d.available === true);
}
...
function findUserByEmail(email) {
  let name = null;
  for (i in spaceData.users) {
    if (spaceData.users[i].email === email) {
      name = spaceData.users[i].name;
    }
  }
  return name;
}
...
function countTotalPassengers() {
  ...
  for (i in spaceData.bookings) {
    totalpassengers += spaceData.bookings[i].passengers.length;
  }
